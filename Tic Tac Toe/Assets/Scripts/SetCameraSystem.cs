    ...
    internal class SetCameraSystem : IEcsRunSystem
    {
        ...
        public void Run()
        {
            if (!_filter.IsEmpty())
            {
                ...
                SceneData.CameraTransform.position =
                    new Vector3(_configuration.LevelWidth / 2f + (_configuration.LevelWidth - 1) * _configuration.Offset.x / 2,
                        _configuration.LevelHeight / 2f + (_configuration.LevelHeight  - 1) * _configuration.Offset.y / 2);
            }
        }
    }
}